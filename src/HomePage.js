      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1.5rem", flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ fontSize: "0.88rem", color: "#64748b", margin: 0, lineHeight: 1.6 }}>
              {weeklyDiff > 0
                ? `You solved ${user.weeklyCount} problems this week — ${weeklyDiff} more than last week. Keep the momentum!`
                : user.weeklyCount > 0
                ? `You've solved ${user.weeklyCount} problems this week. Keep going!`
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px", marginTop: "1.5rem" }}>
          {[
            ...
            { label: "Accuracy",         val: `${accuracyPct}%`,             sub: "correct on first try" },
            { label: "This week",        val: user.weeklyCount || 0,         sub: weeklyDiff >= 0 ? `+${weeklyDiff} vs last week` : `${weeklyDiff} vs last week` },
          ].map((s) => (
...
function DailyChallenge({ challenge, navigate, solvedIds }) {
  const alreadySolved = solvedIds.has(challenge.id);
  const [done, setDone] = useState(alreadySolved);
...
function ProgressFeedback({ user }) {
  ...
  const xp = (user.solvedCount || 0) * 10;
...
// ─── LEADERBOARD CARD ─────────────────────────────────────────────────────────
// Ranked by total solved + current streak combined (fetched live from Supabase)

function LeaderboardCard({ currentUser, leaderboard }) {
  const myRank = leaderboard.findIndex((u) => u.userId === currentUser.userId) + 1;

  return (
    <Card style={{ padding: "0" }}>
      <div style={{ padding: "1rem 1.375rem 0.75rem", borderBottom: "1px solid #f1f5f9" }}>
        <div style={{ fontSize: "0.68rem", color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em" }}>Top solvers (solved + streak)</div>
      </div>
      ...
      {leaderboard.slice(0, 5).map((u, i) => {
        ...
        return (
          <div key={u.userId} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "0.7rem 1.375rem", borderBottom: i < 4 ? "1px solid #f1f5f9" : "none", background: isMe ? "#eff6ff" : "transparent" }}>
            <div style={{ width: "22px", textAlign: "center", fontSize: "0.82rem" }}>
              {badge || <span style={{ color: "#94a3b8", fontSize: "0.72rem" }}>#{i + 1}</span>}
            </div>
            <div style={{ width: "30px", height: "30px", borderRadius: "50%", background: isMe ? "#2563eb" : "#eff6ff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.65rem", fontWeight: 700, color: isMe ? "#fff" : "#2563eb", flexShrink: 0 }}>
              {getInitials(u.fullName)}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "#0f172a" }}>{isMe ? "You" : u.fullName}</div>
      ...
      {myRank > 5 && (
        <div style={{ margin: "0", padding: "0.75rem 1.375rem", background: "#eff6ff", borderTop: "1.5px solid #bfdbfe", borderRadius: "0 0 12px 12px", display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "22px", textAlign: "center", fontSize: "0.75rem", color: "#2563eb", fontWeight: 700 }}>#{myRank}</div>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: "0.68rem", color: "#60a5fa" }}>🔥 {currentUser.streak}d streak</div>
          </div>
          <div style={{ fontSize: "0.78rem", fontWeight: 700, color: "#1d4ed8" }}>{currentUser.solvedCount || 0} solved</div>
        </div>
      )}

      {leaderboard.length === 0 && (
        <div style={{ padding: "1.5rem", textAlign: "center", fontSize: "0.82rem", color: "#94a3b8" }}>
...
// ─── COMMUNITY FEED ───────────────────────────────────────────────────────────
// Shows the 6 most recent correct submissions across all users

function CommunityFeed({ feed }) {
  if (feed.length === 0) {
    return (
      <Card style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "160px" }}>
        <span style={{ fontSize: "0.82rem", color: "#94a3b8" }}>No activity yet — start solving!</span>
      </Card>
    );
  }
  ...
  return (
    <Card style={{ padding: "0" }}>
      ...
      {feed.map((item, i) => (
        <div key={item.id} style={{ display: "flex", alignItems: "flex-start", gap: "10px", padding: "0.75rem 1.375rem", borderBottom: i < feed.length - 1 ? "1px solid #f1f5f9" : "none" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", gap: "6px", alignItems: "center", marginTop: "4px" }}>
              ...
              <span style={{ fontSize: "0.65rem", color: "#94a3b8" }}>{timeAgo(item.created_at)}</span>
            </div>
          </div>
        </div>
      ))}
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: mySubmissions } = await supabase
        .from("submissions")
        .select("id, problem_id, category, problem_title, status, time_taken_seconds, created_at, updated_at, query")
        .eq("user_id", userId)
      ...
      // 5. Fetch leaderboard: all users' solved counts + streaks
      //    We join profiles + submissions (correct) + user_streaks
      //    Strategy: fetch all correct submissions grouped by user_id,
      //    then fetch streaks, then merge with profiles for names.
      ...
      const userSolvedMap = {};
      (allCorrect || []).forEach(({ user_id, problem_id }) => {
        if (!userSolvedMap[user_id]) userSolvedMap[user_id] = new Set();
        userSolvedMap[user_id].add(problem_id);
      });
      ...
      const profileMap = {};
      (allProfiles || []).forEach((p) => { profileMap[p.id] = p.full_name || "Anonymous"; });

      const streakMap = {};
      (allStreaks || []).forEach((s) => { streakMap[s.user_id] = s.current_streak || 0; });

      // All user IDs that have at least one correct submission
      const allUserIds = Object.keys(userSolvedMap);

      const board = allUserIds.map((uid) => ({
        userId:   uid,
        fullName: profileMap[uid] || "Anonymous",
        solved:   userSolvedMap[uid]?.size || 0,
        streak:   streakMap[uid] || 0,
        score:    (userSolvedMap[uid]?.size || 0) + (streakMap[uid] || 0),
      }));
          ...
          <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: "2rem", marginBottom: "2rem" }}>
            ...
            <div>
              ...
              {["Practice", "Leaderboard", "Community"].map((link) => (
                <div key={link} onClick={() => navigate(link === "Practice" ? "/sql" : `/${link.toLowerCase()}`)} style={{ fontSize: "0.82rem", color: "#64748b", marginBottom: "0.4rem", cursor: "pointer" }}>{link}</div>
              ))}
            </div>
            <div>
