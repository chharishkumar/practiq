      ...
      <span>
        Return ONLY these columns. Additional columns will be marked incorrect.
          ...
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "0.625rem 0.75rem", marginBottom: "0.875rem" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.78rem", color: "#1e40af", lineHeight: 1.6 }}>{p.intermediate}</p>
          </div>
          <div style={{ marginBottom: "0.875rem" }}>
...
export default function SQLIntermediatePage() {
  ...
  usePageMeta({
    title: selectedProblem
      ...
      : "SQL Intermediate Practice | Repractiq",
    ...
    canonical: selectedProblem
      ? `https://www.repractiq.com/sql/intermediate/${selectedProblem.id}-${selectedProblem.slug}`
      : "https://www.repractiq.com/sql/intermediate"
  });
...
if (solvedCount < 25) setExpandedMilestone("bronze");
  ...
  const handleSelectProblem = useCallback((p) => {
    ...
    navigate(`/sql/intermediate/${p.id}-${p.slug}`);
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    if (incoming.focusProblemId !== undefined) {
      ...
      if (!isNaN(idFromUrl)) {
        const target = SQL_INTERMEDIATE_PROBLEMS.find((p) => p.id === idFromUrl);
        if (target) {
  ...
  const filteredProblems = useMemo(() => {
    if (!searchTerm.trim()) return SQL_INTERMEDIATE_PROBLEMS;
    return SQL_INTERMEDIATE_PROBLEMS.filter((p) => matchesProblem(p, searchTerm));
  }, [searchTerm]);
