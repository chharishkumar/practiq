      ...
      <span>
        Return ONLY these columns. Additional columns will be marked incorrect.
          ...
          <div style={{ background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "0.625rem 0.75rem", marginBottom: "0.875rem" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.78rem", color: "#1e40af", lineHeight: 1.6 }}>{p.advanced}</p>
          </div>
          <div style={{ marginBottom: "0.875rem" }}>
...
export default function SQLAdvancedPage() {
  ...
  const [selectedProblem, setSelectedProblem] = useState(SQL_ADVANCED_PROBLEMS[0]);
  const [query, setQuery] = useState(SQL_ADVANCED_PROBLEMS[0].starterQuery);
  ...
  usePageMeta({
    title: selectedProblem
      ...
      : "SQL Advanced Practice | Repractiq",
  
    description: selectedProblem
      ? selectedProblem.metaDescription
      : "Practice SQL Advanced interview questions.",
  
    canonical: selectedProblem
      ? `https://www.repractiq.com/sql/advanced/${selectedProblem.id}-${selectedProblem.slug}`
      : "https://www.repractiq.com/sql/advanced"
  });
...
if (solvedCount < 25) setExpandedMilestone("bronze");
  ...
  const handleSelectProblem = useCallback((p) => {
    ...
    navigate(`/sql/advanced/${p.id}-${p.slug}`);
  }, [navigate]);
  ...
  useEffect(() => {
    ...
    if (incoming.focusProblemId !== undefined) {
      const targetProblem = SQL_ADVANCED_PROBLEMS.find((p) => p.id === incoming.focusProblemId);
      if (targetProblem) {
      ...
      if (!isNaN(idFromUrl)) {
        const target = SQL_ADVANCED_PROBLEMS.find((p) => p.id === idFromUrl);
        if (target) {
  ...
  const filteredProblems = useMemo(() => {
    if (!searchTerm.trim()) return SQL_ADVANCED_PROBLEMS;
    return SQL_ADVANCED_PROBLEMS.filter((p) => matchesProblem(p, searchTerm));
  }, [searchTerm]);

  const crossCategoryMatches = useMemo(
    () => searchSqlProblems(searchTerm).filter((m) => m.categoryKey !== "advanced").slice(0, 10),
    [searchTerm]
  );
  ...
  return (
    ...
    />
      ...
      <nav style={{ padding: "0.85rem 2rem", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(255,255,255,0.97)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: "16px", alignItems: "center" }}>
          ...
          <div style={{ fontSize: "0.78rem", color: "#16a34a", background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "20px", padding: "4px 12px", fontWeight: 600 }}>
            ✓ {solvedIds.size} / {SQL_ADVANCED_PROBLEMS.length} attempted
          </div>
      ...
      <div style={{ background: "linear-gradient(180deg, #eff6ff 0%, #ffffff 100%)", borderBottom: "1px solid #e2e8f0", padding: "0.875rem 2rem", display: "flex", alignItems: "center", gap: "16px", flexShrink: 0 }}>
        <h2 style={{ margin: 0, fontSize: "1rem", fontWeight: 800, letterSpacing: "-0.3px", color: "#0f172a" }}>SQL Advanced</h2>
      </div>
          ...
          {!!searchTerm && (
            <div style={{ margin: "0 0.75rem 0.5rem", background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "10px", padding: "0.625rem 0.75rem" }}>
              <div style={{ fontSize: "0.72rem", color: "#1d4ed8", fontWeight: 700, marginBottom: "4px" }}>
                Results: {filteredProblems.length} in Advanced
              </div>
...
{searchTerm ? (
  ...
  <>
    {MILESTONES.map((milestone) => {
      const milestoneProblems = SQL_ADVANCED_PROBLEMS.filter(
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "6px" }}>
                      <span style={{ fontSize: "0.7rem", padding: "3px 10px", borderRadius: "10px", background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0", fontWeight: 600 }}>Advanced</span>
